...
Keep one required check with a stable name such as `ci-required`. Let it validate the results of conditional jobs. Avoid requiring every conditional platform or security job separately. In an automatically triggered workflow, create it for every relevant event; in an agent-gated workflow, create it only during explicit validation and deliberately let its absence on a new PR head block merging before dispatch.
